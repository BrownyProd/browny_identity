...
function SetFirstName(identifier, firstName)
	MySQL.Async.execute('UPDATE `identity` SET `firstname` = @firstname WHERE identifier = @identifier', {
		['@identifier']		= identifier,
		['@firstname']		= firstName
	})
end
function SetLastName(identifier, lastName)
	MySQL.Async.execute('UPDATE `identity` SET `lastname` = @lastname WHERE identifier = @identifier', {
		['@identifier']		= identifier,
		['@lastname']		= lastName
	})
end
function SetDOB(identifier, dob, callback)
	MySQL.Async.execute('UPDATE `identity` SET `dateofbirth` = @dateofbirth WHERE identifier = @identifier', {
		['@identifier']		= identifier,
		['@dateofbirth']	= dob
	})
end
function SetSex(identifier, sexValue)
	MySQL.Async.execute('UPDATE `identity` SET `sex` = @sex WHERE identifier = @identifier', {
		['@identifier']		= identifier,
		['@sex']		    = sexValue
	})
end
function SetHeight(identifier, height)
	MySQL.Async.execute('UPDATE `identity` SET `height` = @height WHERE identifier = @identifier', {
		['@identifier']		= identifier,
		['@height']		    = height
	})
end
